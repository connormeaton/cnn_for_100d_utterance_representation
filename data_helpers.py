...
## original function from Github, replacing with mine below
...
# def load_data_and_labels(positive_data_file, negative_data_file):
    """
    New function to input IMEOCAP data instead of IMDB data. 
    First pass:
        - X = IMEOCAP
        - y = IMDB
        - UPDATE: It works! Now on to expanding categories in model
    """
    '''
    # # Load data from files
    ...
    #         # sent = sent.replace(' ?', '?')
    #         # sent = sent.replace(' !', '!')
    #         # sent = sent.replace('gon na', 'gonna')
